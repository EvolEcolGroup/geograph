...
This document outlines how to contribute to the development of `geoGraph`. The
package is maintained on a voluntary basis, so help is always appreciated. 

## The basic process of contributing

Development work for `geoGraph` occurs in the `dev` branch. So, if you want to
...
whether you should add an appropriate unit test. `geoGraph` uses the `testthat` 
...
- **Functions**: `camelCase` (e.g. `assignByPolygon`)
