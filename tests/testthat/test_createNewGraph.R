test_that("createNewGraph errors on invalid `spacing`", {
  ...
  expect_error(createNewGraph(geo_box, spacing = -1),
               "`spacing` must be a single positive numeric value")
  
  expect_error(createNewGraph(geo_box, spacing = 0),
               "`spacing` must be a single positive numeric value")
  
  expect_error(createNewGraph(geo_box, spacing = NA_real_),
               "`spacing` must be a single positive numeric value")
  
  expect_error(createNewGraph(geo_box, spacing = "1000"),
               "`spacing` must be a single positive numeric value")
  
  expect_error(createNewGraph(geo_box, spacing = c(500, 1000)),
               "`spacing` must be a single positive numeric value")
})
...
test_that("createNewGraph errors on invalid `geo_box`", {
  ...
  expect_error(createNewGraph(c(-10, 30, 35, 60), spacing = 1000),
  ...
  expect_error(createNewGraph(list(xmin = -10, xmax = 30, ymin = 35, ymax = 60), spacing = 1000),
               "geo_box must be a bbox, sf object, or named numeric vector")
})
...
test_that("createNewGraph node IDs are sequential character strings", {
  geo_box <- c(xmin = -10, xmax = 30, ymin = 35, ymax = 60)
  result <- createNewGraph(geo_box, spacing = 1000)
  
  n      <- nrow(result@coords)
  expected_ids <- as.character(seq_len(n))
  actual_ids   <- graph::nodes(result@graphNEL)
  
  expect_equal(sort(actual_ids), sort(expected_ids))
})
