...
test_that("gPath2dist auto-detects dist for between and vector for from", {
  ...
  res.between <- gPath2dist(mb)
  res.from <- gPath2dist(mf)

  ## dijkstraBetween to dist
  expect_s3_class(res.between, "dist")
  expect_equal(attr(res.between, "Size"), 5L)
  expect_equal(length(res.between), 10L)

  ## dijkstraFrom to named numeric vector
  expect_true(is.numeric(res.from))
  expect_false(inherits(res.from, "dist"))
  expect_equal(length(res.from), 10L)
  expect_false(is.null(names(res.from)))
})
