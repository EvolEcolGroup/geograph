test_that(".zoomlog.up rejects malformed input", {
  expect_error(
    geoGraph:::.zoomlog.up("not numeric"),
    "Updating zoomlog using a wrong value"
  )
  expect_error(
    geoGraph:::.zoomlog.up(c(1, 2, 3)),
    "Updating zoomlog using a wrong value"
  )
  expect_error(
    geoGraph:::.zoomlog.up(c("a", "b", "c", "d")),
    "Updating zoomlog using a wrong value"
  )
})
...
test_that(".zoomlog.up caps the zoom history at 100 rows", {
  ...
  original.log <- get("zoom.log", envir = env)
  on.exit(assign("zoom.log", original.log, envir = env), add = TRUE)
...
test_that("geo.bookmark lists, adds, and overwrites bookmarks", {
  env <- geoGraph:::.geoGraphEnv
  original.bookmarks <- get("bookmarks", envir = env)
  on.exit(assign("bookmarks", original.bookmarks, envir = env), add = TRUE)
...
test_that("geo.goto handles unknown and known bookmarks", {
  ...
  on.exit(
    {
      assign("bookmarks", original.bookmarks, envir = env)
      assign("zoom.log", original.log, envir = env)
    },
    add = TRUE
  )
