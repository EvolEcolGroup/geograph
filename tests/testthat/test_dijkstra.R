test_that("dijkstra_between computes distances correctly", {
  ...
  hgdp.between <- dijkstraBetween(hgdp.sub)
  dist.matrix <- gPath2dist(hgdp.between)
  # expect distance between South America and any other location to be
  # to be larger than any other distance
  expect_true(min(dist.matrix[1:3]) > max(dist.matrix[4:6]))

  # now test the same for the gGraph algorithm
  # we extract the nodes from the gData object
  graph.between <- dijkstraBetween(worldgraph.40k,
    from = hgdp.sub@nodes.id,
    to = hgdp.sub@nodes.id
  )
  graph.dist.matrix <- gPath2dist(graph.between)
  expect_true(identical(dist.matrix, graph.dist.matrix))
})
...
testthat::test_that("DijkstraFrom works on a connected graph", {
  ...
  # node 67
  origin <- "67"
  foo <- dijkstraFrom(max.set, origin)
  ...
  testthat::expect_equal(length(names(foo)), nrow(coords.max.set) - 1)
})
...
testthat::test_that("DijkstraFrom works on a gData object", {
  ...
  myPath <- dijkstraFrom(hgdp.sub, start)
...
testthat::test_that("DijkstraBetween can handle two points on the same node in a gData object", {
  ...
  for (i in seq_along(pop)) {
    for (j in seq_along(pop)) {
      if (i != j && nodes.id[i] == nodes.id[j]) {
        testthat::expect_equal(d.mat[i, j], 0)
      }
    }
  }
})
...
test_that("dijkstraFrom errors when start node is not in the graph", {
  ...
  start <- setdiff(getNodes(worldgraph.10k), getNodes(max.set))[1]
  expect_error(
    dijkstraFrom(max.set, start),
