...
the rest of the landmasses. For this we simple use the `getCost` function to access the cost rule 
data fame and change the cost associated to coastal cells.
After defining the new cost rules we set the costs using this dataframe using the `setCost` function, and then compute and plot the corresponding shortest paths:
```{r }
