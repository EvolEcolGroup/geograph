...
the rest of the landmasses. For this we simple use the `getCosts` function to access the cost rule
data frame and change the cost associated to coastal cells.
After defining the new cost rules we set the costs using this dataframe using the `setCosts` function, and then compute and plot the corresponding shortest paths:
```{r }
