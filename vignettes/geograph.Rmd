...
However, more complex habitat information could be incorporated, for instance from GIS shapefiles (see vignette 'Makeing custom grids in geoGraph', @TODO look in what vignette we will look at this).
...
Now of course depending on the application, we may want to use different grid resolutions, and/or more complex habitat information to define costs of traveling through different habitats. This is illustrated in the vignettes 'Making custom grids' and 'Edit graphs' (see `vignette()` for more information on available vignettes).
