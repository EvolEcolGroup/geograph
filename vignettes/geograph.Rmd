...
If we want to condition connectivity on more than just one variable, we may want to combine cost from different gGrpahs into a single metric. For this example, instead of looking at water cells, we focus on land cells and we want the connectivity between the cells to be based on the productivity of the land, which is given in the gGraph object 'productivity_graph' and on temperature, given in the gGraph 'temperatureGraph'. For now we will just set a random cost to `meanProd` and `temp`. 
