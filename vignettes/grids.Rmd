...
sea_cost <- 10
land_cost <- 1
water_land_graph@meta$costs <- data.frame(habitat = c("sea", "land"),
...
rugged_cost <- 3
very_rugged_cost <- 10
elevation_graph@meta$costs <- data.frame(habitat = c("sea", "land", "rugged", "very_rugged"),
...
very_rugged_cost <- 10
coastal_cost <- 1
full_graph@meta$costs <- data.frame(habitat = c("sea", "land", "rugged", "very_rugged", "coast"),
...
```{r}
test <- polygonBetween(newGraph, layer="name", "Czechia", "Portugal", outline = TRUE)
plot(newGraph, reset = TRUE)
plot(test)
```
