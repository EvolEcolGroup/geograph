---
title: "Data visualization in geoGraph"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Data visualization in geoGraph}
...
Now if we want to use have more advanced visualization and publication ready plots, we can use the `autoplot` functions that are based on `ggplot2`. Currently there are `autoplot` methods for `gGraph` and `gData` objects that provide a quick way to visualize the data. 
For more customization, there are also `geom_ggraph`, `geom_gdata` and `geom_gpath` functions that allow for more control over the plotting. Under the hood these functions convert the `gGraph`, `gData` and `gPath` objects into `sf` objects that can be plotted using `ggplot2`.
...
```
As the `gGraph` is now sf object, we have more flexibility in the projections we want to plot the graphs in. By default the the `autoplot` function will plot the `gGraph` in a flat Mercator projection, but we can also choose to plot it in `orthographic` mode for it to look like a globe. This can be done by setting the `mode` argument to `"orthographic"` and specifying the center of the view using the `lon0` and `lat0` arguments.

```{r}
autoplot(worldgraph.40k, mode = "orthographic", lon0 = 0, lat0 = 60)
```

Similarly when we want to plot a `gData` object, we can use the `autoplot` function. By default, the linked `gGraph` will be plotted as well. If we want to plot only the `gData` object, we can set the `show.gGraph` argument to `FALSE`. Again, we can use the `coord_sf` function to change the projection of the plot. For example, we can use the Robinson projection by setting the `crs` argument to `"+proj=robin"`.

```{r}
autoplot(hgdp) + coord_sf(crs = "+proj=robin")
```
